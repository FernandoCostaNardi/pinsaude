...
export const CnpjInput = forwardRef<HTMLInputElement, CnpjInputProps>(
  ({ label, value, onChange, error, disabled, ...props }, ref) => {
    ...
    const valid = complete && isValidCnpj(value)
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="relative">
          ...
          {valid && (
            <CheckCircle
              size={16}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-green-500 pointer-events-none"
            />
          )}
          {invalid && !error && (
            <XCircle
              size={16}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-red-500 pointer-events-none"
            />
          )}
        </div>
        {(error || (invalid && !error)) && (
          <p className="text-xs text-red-500">{error ?? 'CNPJ inválido'}</p>
        )}
      </div>
    )
  }
