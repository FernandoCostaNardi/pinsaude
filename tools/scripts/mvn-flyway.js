...
const goal = process.argv[3] || 'migrate';
if (!module_) { console.error('Usage: node mvn-flyway.js <module> [goal]'); process.exit(1); }
...
if (!mvnPath) { console.error('Maven not found.'); process.exit(1); }
...
console.log(`> mvn flyway:${goal} -pl ${module_}`);
const result = spawnSync(mvnPath, [`flyway:${goal}`, '-pl', module_], {
  cwd: workspaceRoot,
  stdio: 'inherit',
  shell: true,
